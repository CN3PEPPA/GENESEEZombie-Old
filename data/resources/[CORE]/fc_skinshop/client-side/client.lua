...
vSERVER = Tunnel.getInterface("fc_skinshop")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local cam = -1
...
-- SKINDATA
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("fc_skinshop:skinData")
AddEventHandler("fc_skinshop:skinData",function(status)
...
AddEventHandler("updateRoupas",function(custom)
	skinData = custom
	resetClothing(skinData)
	vSERVER.updateClothes(json.encode(custom),false)
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- LOCATESHOPS
-----------------------------------------------------------------------------------------------------------------------------------------
local locateShops = {
	...
	-- { 380.26,-1608.79,29.3 },
	-- { 300.67,-597.63,43.3 },
	-- { 831.06,-1283.25,24.33 },
	-- { 846.12,-1283.83,24.33 },
	-- { 128.64,-757.32,242.16 },
	-- { -438.28,-307.68,34.92 },
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADSYSTEM
...
RegisterCommand("lroupas",function(source,args)
local timeDistance = 500
local ped = PlayerPedId()
	if vSERVER.checkPermissao() then
		timeDistance = 4
		if vSERVER.checkOpen() then
			customCamLocation = nil
			openMenu({
				{ menu = "character", label = "Roupas", selected = true },
				{ menu = "accessoires", label = "Utilidades", selected = false }
...
-- SETMASK
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("fc_skinshop:setMask")
AddEventHandler("fc_skinshop:setMask", function(mask)
    local ped = PlayerPedId()
...
-- SETHAT
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("fc_skinshop:setHat")
AddEventHandler("fc_skinshop:setHat",function(hat)
    local ped = PlayerPedId()
...
-- SETGLASSES
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("fc_skinshop:setGlasses")
AddEventHandler("fc_skinshop:setGlasses", function(glasses)
    local ped = PlayerPedId()
