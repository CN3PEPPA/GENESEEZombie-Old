...
QBClothing.ResetItemTexture = function(obj, category) {
	...
	$.post('http://fc_skinshop/updateSkin', JSON.stringify({
...
$(document).on('change', '.item-number', function(){
	...
	if (clothingCategory == "accessory" && inputVal == 13) {
		...
		$.post('http://fc_skinshop/updateSkinOnInput', JSON.stringify({
			clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
	...
	if (selectedCam == null) {
		...
		$.post('http://fc_skinshop/setupCam', JSON.stringify({
			value: camValue
		}));
		selectedCam = this;
	} else {
...
$(document).on('keydown', function() {
	switch(event.keyCode) {
        case 68: // D
        $.post('http://fc_skinshop/rotateRight');
        break;
        case 65: // A
        $.post('http://fc_skinshop/rotateLeft');
        break;
        case 38: // UP
        ChangeUp();
        break;
...
$(document).on('click', "#save-menu", function(e){
	e.preventDefault();
	QBClothing.Close();
	$.post('http://fc_skinshop/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
	e.preventDefault();
	QBClothing.Close();
	$.post('http://fc_skinshop/resetOutfit');
});
...
$(document).on('click', '.change-camera-button', function(e){
	...
	$.post('http://fc_skinshop/rotateCam', JSON.stringify({
		type: rotationType
	}))
});