	...
	$('#inventory-items').droppable({
		...
		drop: function(event,ui){
			itemData = { 
				...
				amount: ui.draggable.data('item-amount')
			};
			...
			if (amount === undefined || amount === 0 || amount === null || amount === ''){
...
const updateChest = () => {
	$.post("http://vrp_chest/requestChest",JSON.stringify({}),(data) => {
		...
		$('#inventory-items').html(`
			${nameList2.map((item) => (`
				<div class="slot">
					<div class="item" data-item-key="${item.key}" data-item-amount="${item.amount}">
		...
		$('#chest-items').html(`
			${nameList.map((item) => (`
			<div class="slot">
				<div class="chest-item" data-item-key="${item.key}" data-item-amount="${item.amount}">
					<div id="item-icon"><img src='${data.imageService}/${item.index}.png'></div>
					<div id="item-weight">${(item.peso*item.amount).toFixed(2)}Kg</div>
					<div id="item-amount">${formatarNumero(item.amount)}x</div>
					<div id="item-name">${item.name}</div>
