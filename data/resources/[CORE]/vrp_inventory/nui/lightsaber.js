$(document).ready(function () {
	...
	$(document).on('mousedown', '.item', function (ev) {
		if (ev.which == 3) {
			itemData = {
				key: $(ev.target).closest('.item').data('item-key'),
				type: $(ev.target).closest('.item').data('item-type')
			};
	...
	$.post("http://vrp_inventory/requestInventory", JSON.stringify({}), (data) => {
		...
		$('#inventory-amount').html(`
            <div class="amount-bar"><span id="amount-bar" style="width: ${
				...
				(data.peso*100/data.maxpeso).toFixed(2) <= 100 ? "rgba(131, 5, 5, 1.0)" : "rgb(236, 48, 48)"
			};"></span></div>
