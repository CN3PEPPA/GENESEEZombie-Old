...
local config = module("genesee/others")
...
function vRP.setSDataChest(key, value)
    vRP.execute("vRP/set_srvdatachest", {
        user_id = key,
        value = value
    })
end

function vRP.getSDataChest(key, cbr)
    local rows = vRP.query("vRP/get_srvdatachest", {
        user_id = key
    })
    if #rows > 0 then
        return rows[1].value
...
function vRP.dropPlayer(source)
    ...
    if user_id then
        if user_id and source then
            ...
            PerformHttpRequest(base.exit, function(err, text, headers)
                ...
                embeds = {{
                    ...
                    footer = {
                        text = base.bottomText .. os.date("%d/%m/%Y | %H:%M:%S"),
                        icon_url = base.icon
                    },
                    color = base.color
                }}
            ...
            if vRP.hasGroup(user_id, "administrador") then
                ...
                PerformHttpRequest(base.staff, function(err, text, headers)
                end, 'POST', json.encode({
                    embeds = {{
                        ...
                        fields = {{
                            name = "**IDENTIFICAÇÃO: " .. identity.name .. " " .. identity.firstname .. "** [**" ..
                                user_id .. "**]",
                            value = "⠀"
                        }, {
                            name = "**CARGO: **" .. office,
        ...
        if user_id then
            ...
            if not vRP.isBanned(user_id) then
                ...
                if vRP.isWhitelisted(user_id) then
                    vRP.execute("vRP/update_login", {
                        ll = os.date("%H:%M:%S %d/%m/%Y"),
                        uid = user_id
                    })
                    vRP.execute("vRP/update_ip", {
                    ...
                    if vRP.rusers[user_id] == nil then
                        ...
                        PerformHttpRequest(base.join, function(err, text, headers)
                        end, 'POST', json.encode({
                            embeds = {{
                    ...
                    deferrals.done(
                        "Você não possui visto na cidade! [ Adquira seu visto em: discord.gg/projetozrp ][ ID: " ..
                            user_id .. " ]")
                    TriggerEvent("queue:playerConnectingRemoveQueues", ids)
                end
            else
                deferrals.done("Você foi banido! [ Mais informações em: discord.gg/projetozrp ] ")
                TriggerEvent("queue:playerConnectingRemoveQueues", ids)
            end
        else
