...
function tvRP.vehList(radius)
    ...
    if IsEntityAVehicle(veh) then
        ...
        for k, v in pairs(vehs.vehglobal) do
            if v.hash == GetEntityModel(veh) then
                if k then
                    local tuning = {GetNumVehicleMods(veh, 13), GetNumVehicleMods(veh, 12), GetNumVehicleMods(veh, 15),
                                    GetNumVehicleMods(veh, 11), GetNumVehicleMods(veh, 16)}
                    return veh, VehToNet(veh), GetVehicleNumberPlateText(veh), k,
                        GetVehicleDoorsLockedForPlayer(veh, PlayerId()), v.banned, trunk,
                        GetDisplayNameFromVehicleModel(k), GetStreetNameFromHashKey(GetStreetNameAtCoord(x, y, z)),
                        tuning
                end
            end
        end
