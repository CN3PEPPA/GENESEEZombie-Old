...
function tvRP.getCanceling()
    return canceling
end

RegisterNetEvent('canceling')
AddEventHandler('canceling', function(status)
    canceling = status
end)
...
function tvRP.loadObject(dict, anim, prop, flag, hand, pos1, pos2, pos3, pos4, pos5, pos6)
...
function tvRP.deleteObject()
    tvRP.stopAnim(true)
    TriggerEvent("binoculos")
    if DoesEntityExist(object) then
        TriggerServerEvent("GENESEESync:trydeleteobj", ObjToNet(object))
        object = nil
    end
end
...
RegisterCommand('vrp:up', function()
    if menu_state.opened then
        ...
        local ped = PlayerPedId()
        if not IsPedInAnyVehicle(ped) and GetEntityHealth(ped) > 101 and not menu_state.opened and not canceling and
...
RegisterCommand('vrp:cancelAnims', function()
    ...
    if cooldown < 1 then
        ...
        if GetEntityHealth(ped) > 101 then
            if not menu_state.opened and not canceling then
                tvRP.deleteObject()
                ClearPedTasks(ped)
            end
        end
...
RegisterCommand('vrp:anim10', function()
    local ped = PlayerPedId()
    if GetEntityHealth(ped) > 101 and not menu_state.opened and not canceling and not IsPauseMenuActive() then
...
RegisterCommand('vrp:animX', function()
    local ped = PlayerPedId()
    if not IsPedInAnyVehicle(ped) and GetEntityHealth(ped) > 101 and not canceling and not IsPauseMenuActive() then
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(0, 29) then
            if GetEntityHealth(ped) > 101 and not canceling then
                tvRP.loadAnim("anim@mp_point")
                if not apontar then
