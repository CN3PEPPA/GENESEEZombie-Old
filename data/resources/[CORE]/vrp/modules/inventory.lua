...
local itemlist = items.list
function vRP.returnItems()
    return itemlist
end
...
function vRP.getRemaingTrunkSlots(chestData, chestSlots)
...
function vRP.getRemaingChestSlots(chestData, chestSlots)
    ...
    local data = vRP.getSDataChest(chestData)
    local result = json.decode(data) or {}
...
function vRP.storeTrunkItem(user_id, chestData, itemName, amount, chestWeight, chestSlots)
    if actived[user_id] == nil then
        ...
        if data and items ~= nil then
            ...
            local new_weight = vRP.computeItemsWeight(items) + vRP.getItemWeight(itemName) *
                                   parseInt(activedAmount[user_id])
            ...
            if items[itemName] ~= nil then
                ...
                if new_weight <= parseInt(chestWeight) and vRP.getRemaingTrunkSlots(chestData, chestSlots) >= 1 then
                    if vRP.tryGetInventoryItem(parseInt(user_id), itemName, parseInt(activedAmount[user_id])) then
                        items[itemName] = {
                            amount = parseInt(activedAmount[user_id])
                        }
                        vRP.setSData(chestData, json.encode(items))
                        return true
                    end
        ...
        if data and items ~= nil then
            ...
            if items[itemName] ~= nil then
                if new_weight <= parseInt(chestWeight) then
                    if vRP.tryGetInventoryItem(parseInt(user_id), itemName, parseInt(activedAmount[user_id])) then
                        items[itemName].amount = parseInt(items[itemName].amount) + parseInt(activedAmount[user_id])
                        vRP.setSDataChest(chestData, json.encode(items))
                        return true
                    end
                end
            else
...
function vRP.tryTrunkItem(user_id, chestData, itemName, amount)
    if actived[user_id] == nil then
        actived[user_id] = 1
        local data = vRP.getSData(chestData)
        local items = json.decode(data) or {}
        if data and items ~= nil then
            if items[itemName] ~= nil and parseInt(items[itemName].amount) >= parseInt(amount) then
                ...
                local new_weight = vRP.getInventoryWeight(parseInt(user_id)) + vRP.getItemWeight(itemName) *
                                       parseInt(activedAmount[user_id])
                ...
                if vRP.checkExistentItem(user_id, itemName) then
                    ...
                    if new_weight <= vRP.getInventoryMaxWeight(parseInt(user_id)) and
                        vRP.getRemaingSlots(parseInt(user_id)) >= 1 then
                        vRP.giveInventoryItem(parseInt(user_id), itemName, parseInt(activedAmount[user_id]))
