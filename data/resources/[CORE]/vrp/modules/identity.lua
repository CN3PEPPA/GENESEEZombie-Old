function vRP.getUserIdentity(user_id, cbr)
    local rows = vRP.query("vRP/get_user_identity", {
        user_id = user_id
    })
    return rows[1]
end

function vRP.getUserDiscord(user_id, cbr)
    local rows = vRP.query("vRP/get_user_discord", {
        user_id = user_id
    })
...
function vRP.getUserByRegistration(registration, cbr)
    local rows = vRP.query("vRP/get_userbyreg", {
