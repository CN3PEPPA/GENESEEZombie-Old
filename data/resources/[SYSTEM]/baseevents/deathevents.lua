        ...
        if NetworkIsPlayerActive(player) then
            ...
            if IsPedFatallyInjured(ped) and not isDead then
                ...
                if killer == ped or killer == -1 then
                    TriggerEvent('baseevents:onPlayerDied', PlayerId(), { table.unpack(GetEntityCoords(ped)) })
                    TriggerServerEvent('baseevents:onPlayerDied', PlayerId(), { table.unpack(GetEntityCoords(ped)) })
                    hasBeenDead = true
                else
