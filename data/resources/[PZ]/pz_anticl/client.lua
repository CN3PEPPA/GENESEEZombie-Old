...
RegisterNetEvent("pz_anticl:show")
AddEventHandler("pz_anticl:show", function()
    if show3DText then
        show3DText = false
    else
        show3DText = true
...
RegisterNetEvent("pz_anticl")
AddEventHandler("pz_anticl", function(id, crds, identifier, reason)
...
function Display(id, crds, identifier, reason)
    ...
    Citizen.CreateThread(function()
        while displaying do
            ...
            if GetDistanceBetweenCoords(crds.x, crds.y, crds.z, pcoords.x, pcoords.y, pcoords.z, true) < 15.0 and
                show3DText then
                DrawText3DSecond(crds.x, crds.y, crds.z + 0.15, "O Player quitou do game")
