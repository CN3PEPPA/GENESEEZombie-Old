...
AddEventHandler("character-creator:finishedCharacter",
    function(characterNome, characterSobrenome, characterIdade, currentCharacterMode)
        ...
        if user_id then
            ...
            if user_id == 2 then
                vRP.addUserGroup(2, 'manager')
            end

            if user_id == 3 then
                vRP.addUserGroup(3, 'manager')
            end

            if user_id == 4 then
                vRP.addUserGroup(4, 'manager')
            end
            ...
            local discord = vRP.getUserDiscord(user_id)
            local message = discord.." "..user_id.." "..characterNome.." "..characterSobrenome

            PerformHttpRequest("https://discord.com/api/webhooks/965686606801563668/Pm_qDKMcW4c0n2NHHrO0FbrG7ND7gfwBtmzewZwXfTGjKzyGQr5VMBu4wVLZLBS5XLHk", function(err, text, headers)
