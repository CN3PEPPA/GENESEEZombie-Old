...
function TriggerCreateCharacter()
    ...
    SetEntityCoordsNoOffset(PlayerPedId(), config.creatorCoord[1], config.creatorCoord[2], config.creatorCoord[3], true,
        true, true)
    ...
    SendNUIMessage({
        CharacterMode = isInCharacterMode,
        CharacterMode2 = not isInCharacterMode,
        CharacterMode3 = not isInCharacterMode
    })
    EndFade()
end
...
RegisterNUICallback('cDoneSave', function(data, cb)
    ...
    SendNUIMessage({
        CharacterMode = isInCharacterMode,
        CharacterMode2 = isInCharacterMode,
        CharacterMode3 = isInCharacterMode
    })
    if GetEntityModel(PlayerPedId()) == GetHashKey("mp_m_freemode_01") then
        SetPedComponentVariation(PlayerPedId(), 1, -1, 0, 2)
        SetPedComponentVariation(PlayerPedId(), 3, 15, 0, 2)
    ...
    SetEntityCoordsNoOffset(PlayerPedId(), coords[location].x, coords[location].y, coords[location].z, true, true, true)
    ...
    TriggerServerEvent("character-creator:finishedCharacter", characterNome, characterSobrenome, characterIdade,
...
function TaskUpdateSkinOptions()
    ...
    SetPedHeadBlendData(PlayerPedId(), data.fathersID, data.mothersID, 0, data.skinColor, 0, 0, f(data.shapeMix), 0, 0,
        false)
end
