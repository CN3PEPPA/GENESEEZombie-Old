...
```lua
exports['target']:AddBoxZone("MissionRowDutyClipboard", vector3(441.7989, -982.0529, 30.67834), 0.45, 0.35, {
...
exports['target']:AddTargetModel(Config.Peds, {
	options = {
		{
...
```lua
exports['target']:AddTargetEntity('mule2', {
    options = {
        {
            type = "client",
...
## Passing Item Data
In this example, we define the model of the coffee machines you see around the map, and allow players to purchase a coffee. You'll have to provide your own logic for the purchase, but this is how you would handle it with target, and how you would pass data through to an event for future use.
...
AddEventHandler('plantpotato',function()
	...
	SetEntityInvincible(plant, false)

	-- Logic to handle growth, create a thread and loop, or do something else. Up to you.

	exports['target']:AddEntityZone("potato-growing-"..plant, plant, {
