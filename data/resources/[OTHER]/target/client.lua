...
local function ScreenPositionToCameraRay()
    ...
    return pos,
        glm_rayPicking(q * glm_forward, q * glm_up, glm_rad(screen.fov), screen.ratio, 0.10000, -- GetFinalRenderedCamNearClip(),
...
local function RaycastCamera(flag, playerCoords)
    ...
    local rayHandle = StartShapeTestLosProbe(rayPos.x, rayPos.y, rayPos.z, destination.x, destination.y, destination.z,
        flag or -1, playerPed, 0)
    while true do
        local result, _, endCoords, _, entityHit = GetShapeTestResult(rayHandle)
        if Config.Debug then
            DrawLine(playerCoords.x, playerCoords.y, playerCoords.z, destination.x, destination.y, destination.z, 255,
                0, 255, 255)
            DrawLine(destination.x, destination.y, destination.z, endCoords.x, endCoords.y, endCoords.z, 255, 0, 255,
                255)
        end
...
local function EnableNUI(options)
    if not targetActive or hasFocus then
        return
    end
...
local function DisableTarget(forcedisable)
    if (not targetActive and hasFocus and not Config.Toggle) or not forcedisable then
        return
    end
    SetNuiFocus(false, false)
    SetNuiFocusKeepInput(false)
...
local function DrawOutlineEntity(entity, bool)
    if not Config.EnableOutline or IsEntityAPed(entity) then
        return
    end
    ...
    if next(nuiData) then
        ...
        while targetActive and success do
            ...
            if entity ~= entity2 then
                ...
                break
            elseif not hasFocus and
                (IsControlPressed(0, Config.MenuControlKey) or IsDisabledControlPressed(0, Config.MenuControlKey)) then
                EnableNUI(nuiData)
                DrawOutlineEntity(entity, false)
            else
...
local function EnableTarget()
    if not AllowTarget or success or (not Config.Standalone and not LocalPlayer.state['isLoggedIn']) or IsNuiFocused() or
        (Config.DisableInVehicle and IsPedInAnyVehicle(playerPed or PlayerPedId(), false)) then
        return
    end
    if not CheckOptions then
        CheckOptions = _ENV.CheckOptions
    end
    if targetActive or not CheckOptions then
        return
    end
    targetActive = true
    SendNUIMessage({
        ...
        if distance <= Config.MaxDistance then
                                ...
                                while targetActive and success do
                                    ...
                                    if entity == entity2 then
                                        ...
                                        if closestBone ~= closestBone2 then
                                            ...
                                            break
                                        elseif not hasFocus and
                                            (IsControlPressed(0, Config.MenuControlKey) or
                                                IsDisabledControlPressed(0, Config.MenuControlKey)) then
                                            EnableNUI(nuiData)
                                            DrawOutlineEntity(entity, false)
                                        else
                                            for k, v in pairs(sendDistance) do
            ...
            if not success then
                ...
                for _, zone in pairs(Zones) do
                    if distance < (closestDis or Config.MaxDistance) and distance <= zone.targetoptions.distance and
                        zone:isPointInside(coords) then
                        closestDis = distance
                        closestZone = zone
                    end
                end
...
local function AddTargetBone(bones, parameters)
    ...
    if type(bones) == 'table' then
        for _, bone in pairs(bones) do
            if not Bones.Options[bone] then
                Bones.Options[bone] = {}
            end
    ...
    elseif type(bones) == 'string' then
        if not Bones.Options[bones] then
            Bones.Options[bones] = {}
        end
        for k, v in pairs(options) do
            if not v.distance or v.distance > distance then
...
local function AddTargetModel(models, parameters)
    ...
    if type(models) == 'table' then
        ...
        if type(models) == 'string' then
            models = GetHashKey(models)
        end
        if not Models[models] then
            Models[models] = {}
        end
        for k, v in pairs(options) do
            if not v.distance or v.distance > distance then
...
function SpawnPeds()
    if PedsReady or not next(Config.Peds) then
        return
    end
...
function DeletePeds()
    if not PedsReady or not next(Config.Peds) then
        return
    end
    for k, v in pairs(Config.Peds) do
...
exports("GetTargetBoneData", function(bone)
    return Bones.Options[bone][label]
end)
...
exports("UpdateGlobalTypeData", function(type, label, data)
