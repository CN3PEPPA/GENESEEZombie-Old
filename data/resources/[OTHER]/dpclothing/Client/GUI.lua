...
local function DrawButton(b)
    ...
    if b.Shadow then
        DrawSprite("dp_clothing", "circle", b.x, b.y, b.Size.Circle.x / 0.80, b.Size.Circle.y / 0.80, Rot, b.Colour.r,
            b.Colour.g, b.Colour.b, b.Alpha)
    end
    DrawSprite("dp_clothing", b.Sprite, b.x, b.y, b.Size.Sprite.x / 0.68, b.Size.Sprite.y / 0.68, b.Rotation, 255, 255,
        255, b.Alpha)
...
local function GenerateTheButtons() -- We generate the buttons here to save on a little bit of performance.
    ...
    for k, v in pairs(Config.Commands) do
        local i = v.Button
        local Angle = i * math.pi / 7
        local Ptx, Pty = x + rx * math.cos(Angle), y + ry * math.sin(Angle)
        Buttons[i] = {
            Command = k,
            Desc = v.Desc or "",
        ...
        for k, v in pairs(Config.ExtraCommands) do
            ...
            ExtraButtons[k] = {
                ...
                Size = {
                    ...
                    Sprite = {
                        x = DefaultButton.x / 1.35,
                        y = DefaultButton.y / 1.35
                    }
                },
                Sprite = v.Sprite,
...
local function PushedButton(button, extra, rotate, info) -- https://www.youtube.com/watch?v=v57i1Ze0jB8
    Citizen.CreateThread(function()
        ...
        if not extra then
            Button.Size = {
                Sprite = {
                    x = DefaultButton.x / 1.1,
                    y = DefaultButton.y / 1.1
                }
            }
            Wait(100)
            Button.Size = {
            ...
            Button.Size = {
                ...
                Sprite = {
                    x = DefaultButton.x / 1.3 / 1.1,
                    y = DefaultButton.y / 1.3 / 1.1
                }
            }
            Wait(100)
            Button.Size = {
...
local function HoveredButton()
    ...
    for k, v in pairs(Buttons) do
        ...
        if Distance < 0.025 then
            ...
            Text(Config.GUI.Position.x, Config.GUI.Position.y - 0.08, 0.22, v.Desc, {210, 210, 210}, false, true, {
                x = 0.1,
                y = 0.2
            })
        end
    ...
    for k, v in pairs(ExtraButtons) do
        if v.Enabled then
            local Distance = Distance(Config.GUI.Position.x + v.OffsetX + 0.005,
                Config.GUI.Position.y + v.OffsetY + 0.025, x, y)
            ...
            if ShouldDisplay then
                if Distance < 0.025 then
                    Text(Config.GUI.Position.x, Config.GUI.Position.y - 0.10, 0.3, v.Text, false, false, true)
                    Text(Config.GUI.Position.x, Config.GUI.Position.y - 0.08, 0.22, v.Desc, {210, 210, 210}, false,
                        true, {
                            x = 0.1,
                            y = 0.2
                        })
...
local function DrawGUI()
    ...
    if Cooldown then
        Text(x, y + 0.05, 0.28, Lang("PleaseWait"), false, false, true)
    end -- Cooldown indicator, if theres a cooldown we display a little text.
    ...
    if InfoButton then
        ...
        for k, v in pairs(LastEquipped) do
            log(k .. " : " .. json.encode(v))
        end -- If the info button is pressed we log all "LastEquipped" items, for debugging purposes.
    end
end
