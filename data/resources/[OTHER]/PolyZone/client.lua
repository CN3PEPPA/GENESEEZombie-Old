...
local function _pointInPoly(point, poly)
    ...
    if x < minX or x > max.x or y < minY or y > max.y then
        return false
    end
...
local function _calculateGridCellPoints(cellX, cellY, poly)
    ...
    return {vector2(x, y), vector2(x + gridCellWidth, y), vector2(x + gridCellWidth, y + gridCellHeight),
            vector2(x, y + gridCellHeight), vector2(x, y)}
end
...
function _isGridCellInsidePoly(cellX, cellY, poly)
    ...
    for i = 1, #gridCellPoints - 1 do
        ...
        if _windingNumber(cellPoint, poly.points) then
            ...
            if poly.lines then
                if not poly.gridXPoints[x] then
                    poly.gridXPoints[x] = {}
                end
                if not poly.gridYPoints[y] then
                    poly.gridYPoints[y] = {}
                end
