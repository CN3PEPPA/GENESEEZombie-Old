...
local function _removeZoneByFunction(predicateFn, zones)
    if predicateFn == nil or zones == nil or #zones == 0 then
        return
    end
...
function ComboZone:RemoveZone(nameOrFn)
    ...
    if type(nameOrFn) == "string" then
        ...
        predicateFn = function(zone)
            return zone.name == nameOrFn
        end
...
function ComboZone:printInfo()
    ...
    if comboCount > 0 then
        print("[PolyZone]   ComboZones: " .. comboCount)
    end
    print("-----------------------------------------------------")
end
