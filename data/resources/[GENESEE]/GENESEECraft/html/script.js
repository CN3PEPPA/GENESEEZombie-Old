...
function closeMenu() {
    $.post('https://GENESEECraft/close', JSON.stringify({}));
...
function reopenMenu() {
    $.post('https://GENESEECraft/reopen', JSON.stringify({}));
    ...
    for (const [key1, value1] of Object.entries(categories)) {
        ...
        if (add) {
            first = first + '    <div class="clearfix colelem recipe" data-category="' + key1 + '" onclick="openCrafting(this)" id="pu212"><!-- group -->' +
                ...
                '      <p>' + value1.Label + '</p>' +
                '     </div>' +
                '     <div class="museBGSize grpelem" id="u264" style="background: url(https://arcadiuscity.com/sources/vrp_images/' + value1.Image + '.png) no-repeat center; background-size: 100%; " data-sizePolicy="fixed" data-pintopage="page_fixedCenter" data-leftAdjustmentDoneBy="pu212"><!-- simple frame --></div>' +
                '    </div>';
        }
    }
            ...
            if (value.Level > level) {
                ...
                if (!hidecraft) {
                    ...
                    second = second + '    <div class="clearfix colelem recipe" onclick="inspect(this)" data-item="' + key + '" style="opacity: 0.5;" id="pu212"><!-- group -->' +
                        ...
                        '     </div>' +
                        '     <div class="museBGSize grpelem" id="u264" style="background: url(https://arcadiuscity.com/sources/vrp_images/' + key + '.png) no-repeat center; background-size: 100%; " data-sizePolicy="fixed" data-pintopage="page_fixedCenter" data-leftAdjustmentDoneBy="pu212"><!-- simple frame --></div>' +
...
function addToQueue(item, time, id) {
    ...
    if ($("#" + id).length) {
        ...
        var base = '    <div class="slide-left queue" id="' + id + '"><!-- group -->' +
            ...
            '     </div>' +
            '     <div class="museBGSize grpelem" style="background: url(https://arcadiuscity.com/sources/vrp_images/' + item + '.png) no-repeat center; background-size: 100%; " id="u546" data-sizePolicy="fixed" data-pintopage="page_fixedCenter" data-leftAdjustmentDoneBy="pu544"><!-- simple frame --></div>' +
...
function craft(t) {
    ...
    $.post('https://GENESEECraft/craft', JSON.stringify({
        item: item
    }));
}
...
function inspect(t) {
    ...
    if (opened != t) {
        ...
        var base = '   <div class="slide-bottom " id="pu386"><!-- group -->' +
            ...
            '    <div class="museBGSize grpelem" id="u389" style="background: url(https://arcadiuscity.com/sources/vrp_images/' + t.dataset.item + '.png) no-repeat center; background-size: 100%; " data-sizePolicy="fixed" data-pintopage="page_fixedCenter" data-leftAdjustmentDoneBy="pu386"><!-- simple frame --></div>' +
        ...
        for (const [key, value] of Object.entries(ingredients)) {
            if (inventory[key] >= value) {
                first = first + '<div class="ingredient" id="' + key + '">' +
                    ...
                    '<div id="ingredient-logo" style="background: url(https://arcadiuscity.com/sources/vrp_images/' + key + '.png) no-repeat center; background-size: 100%; "></div>' +
                    '</div>';
            } else {
                second = second + '<div class="ingredient" id="' + key + '" style="opacity:0.3;">' +
                    '<div id="ingredient-text">' + names[key] + '</div>' +
                    '<div id="ingredient-x-error">' + value + 'X</div>' +
                    '<div id="ingredient-logo" style="background: url(https://arcadiuscity.com/sources/vrp_images/' + key + '.png) no-repeat center; background-size: 100%; "></div>' +
                    '</div>';
            }
        }
