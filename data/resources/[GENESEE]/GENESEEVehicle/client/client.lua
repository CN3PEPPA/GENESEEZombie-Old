...
Proxy.addInterface("GENESEEVehicle", passos)
    ...
    while true do
        ...
        if repairMode then
                    ...
                    if GetVehicleDoorAngleRatio(currentVehicle, 4) ~= 0 then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) and #(pedCoords - coords) < 2.0 then
                            ...
                            Citizen.CreateThread(function()
                                ...
                                TriggerEvent("Notify", "sucesso", Config.Text["mechanic_action_complete"])
                            end)
                        end
                    end
                else
                ...
                if DoesVehicleHaveDoor(currentVehicle, 4) then
                    if GetVehicleDoorAngleRatio(currentVehicle, 4) ~= 0 then
                        ...
                        if not repairing then
                            ...
                            if repairInstall then
                                DrawText3D(coords.x, coords.y, coords.z, Config.Text["installing_sparkplugs"])
                            else
                                DrawText3D(coords.x, coords.y, coords.z, Config.Text["installing_sparkplugs"])
                            end
                        end
            ...
            elseif repairPartType == "transmission" then
                ...
                if vehicleClass == 0 or vehicleClass == 1 or vehicleClass == 2 or vehicleClass == 3 or vehicleClass == 4 or
                    vehicleClass == 5 or vehicleClass == 6 or vehicleClass == 7 then
            ...
            elseif repairPartType == "tires" then
                ...
                local wheel1 = -- GetOffsetFromEntityInWorldCoords(currentVehicle, off.x + 0.9, off.y + 0.6, off.z - 0.4)
                GetWorldPositionOfEntityBone(currentVehicle, bone1)
                local wheel2 = -- GetOffsetFromEntityInWorldCoords(currentVehicle, off.x - 0.9, off.y + 0.6, off.z - 0.4)
                GetWorldPositionOfEntityBone(currentVehicle, bone2)
                local wheel3 = -- GetOffsetFromEntityInWorldCoords(currentVehicle, off.x + 0.9, off.y - 2.27, off.z - 0.4)
                GetWorldPositionOfEntityBone(currentVehicle, bone3)
                local wheel4 = -- GetOffsetFromEntityInWorldCoords(currentVehicle, off.x - 0.9, off.y - 2.27, off.z - 0.4)
                ...
                if IsControlJustReleased(0, Keys["E"]) and #(pedCoords - wheel1) < 1.5 and w1 then
                    ...
                    Citizen.CreateThread(function()
                        ...
                        TaskPlayAnim(ped, 'anim@amb@clubhouse@tutorial@bkr_tut_ig3@', 'machinic_loop_mechandplayer',
...
Citizen.CreateThread(function()
    while true do
        ...
        if seat == ped and not inVehicle then
            ...
            if Vehicle.getIfVehicleOwned() then
        ...
        if inVehicle and currentVehicleParts ~= nil then
            ...
            local mileagehud = math.floor(mileage * 100) / 100
            TriggerEvent('pz_hud:mileage', mileagehud)
            ...
            if currentVehicleParts["engine"].health <= 0 then
                ...
                SetVehicleEngineHealth(currentVehicle,
            ...
            if currentVehicleParts["tires"] then
                ...
                elseif currentVehicleParts["tires"].health > 0 then
                    currentVehicleParts["tires"].health = currentVehicleParts["tires"].health -
                                                              ((100 -
                                                                  Config.Tires[currentVehicleParts["tires"].type]
                                                                      .durability) *
            ...
            if currentVehicleParts["brakes"] then
                ...
                elseif currentVehicleParts["brakes"].health > 0 then
                    ...
                    currentVehicleParts["brakes"].health = currentVehicleParts["brakes"].health -
                                                               ((100 -
                                                                   Config.Brakes[currentVehicleParts["brakes"].type]
                                                                       .durability) *
                                                                   ((kmh / (currentWear * 150)) +
                                                                       (rpm / (currentWear * 10))))
                end
                ...
                if currentVehicleParts["nitro"].health < 0 then
                    ...
                    if nitroPower > 0 and IsControlPressed(0, Keys[Config.NitroKey]) then
                        ...
                        if IsControlPressed(0, 71) then
                            ...
                            SetVehicleEnginePowerMultiplier(currentVehicle,
                        ...
                        if nitroActive then
                            SetVehicleHandlingFloat(currentVehicle, "CHandlingData", "fDriveInertia",
            ...
            if currentVehicleParts["oil"].health <= 0 then
                ...
                currentVehicleParts["oil"].health = currentVehicleParts["oil"].health -
            ...
            if currentVehicleParts["sparkplugs"].health <= 0 then
                ...
                currentVehicleParts["sparkplugs"].health = currentVehicleParts["sparkplugs"].health -
                                                               ((100 -
                                                                   Config.SparkPlugs[currentVehicleParts["sparkplugs"]
                                                                       .type].durability) *
                                                                   ((kmh / (currentWear * 50)) +
                                                                       (rpm / (currentWear * 50))))
            end
...
RegisterNetEvent("core_vehicle:Open")
AddEventHandler("core_vehicle:Open", function(tool)
    local ped = PlayerPedId()
    local coords = GetEntityCoords(ped)

    local workshop = true
    local veh, dst = GetClosestVehicle(coords)
    local plate = GetVehicleNumberPlateText(veh)
    local localVehLockStatus = GetVehicleDoorLockStatus(veh)
    if dst < Config.DetectDistance and Vehicle.getIfVehicleOwned() then
        local source = source
        ...
        if vehicleClass == 0 or vehicleClass == 1 or vehicleClass == 2 or vehicleClass == 3 or vehicleClass == 4 or
            ...
            TriggerServerEvent("core_vehicle:getVehicleParts", string.gsub(currentPlate, "%s+", ""),
                NetworkGetNetworkIdFromEntity(veh), Model(GetEntityModel(currentVehicle)))
            Citizen.Wait(300)
            local parts = {}
            ...
            SendNUIMessage({
                ...
                parts = parts,
                plate = currentPlate,
                vehiclemodel = Model(GetEntityModel(currentVehicle)),
                allparts = allparts,
                mileage = math.floor(mileage * 100) / 100,
                workshop = workshop,
        ...
        TriggerEvent("Notify", "negado", Config.Text["vehicle_nearby"])
    end
end)

RegisterNetEvent("core_vehicle:startInstall")
AddEventHandler("core_vehicle:startInstall", function(partType, part)
    w1, w2, w3, w4 = true, true, true, true
    done = 0

    TriggerEvent("Notify", "aviso", Config.Text["mechanic_action_started"])
    repairMode = true
...
AddEventHandler("core_vehicle:startRepair", function(partType, part, ret)
    w1, w2, w3, w4 = true, true, true, true
    done = 0
...
RegisterNUICallback("removePart", function(data)
    local partType = data["parttype"]
...
RegisterNUICallback("install", function(data)
...
RegisterNUICallback("replace", function(data)
    local part = data["part"]
    local partType = data["parttype"]
...
RegisterNUICallback("cancelrepair", function(data)
    local part = data["part"]
    local partType = data["parttype"]
...
AddEventHandler("core_vehicle:getVehicleParts_c", function(parts, mileage)
    currentMileage = mileage
    SetUpVehicle(parts)
end)

RegisterNetEvent("core_vehicle:getVehicleHandling_c")
AddEventHandler("core_vehicle:getVehicleHandling_c", function(plate, handlingData)
    if handlingData == nil then
        ...
        if Config.UseRelativeValues then
            handling["fClutchChangeRateScaleDownShift"] = GetVehicleHandlingFloat(currentVehicle, "CHandlingData",
                "fClutchChangeRateScaleDownShift")
            handling["fClutchChangeRateScaleUpShift"] = GetVehicleHandlingFloat(currentVehicle, "CHandlingData",
                "fClutchChangeRateScaleUpShift")
            handling["fLowSpeedTractionLossMult"] = GetVehicleHandlingFloat(currentVehicle, "CHandlingData",
                "fLowSpeedTractionLossMult")
            handling["fTractionCurveLateral"] = GetVehicleHandlingFloat(currentVehicle, "CHandlingData",
                "fTractionCurveLateral")
            handling["fBrakeForce"] = GetVehicleHandlingFloat(currentVehicle, "CHandlingData", "fBrakeForce")
            handling["fSuspensionRaise"] = GetVehicleHandlingFloat(currentVehicle, "CHandlingData", "fSuspensionRaise")
...
function setTransmission(veh)
    ...
    if currentVehicleParts["transmission"].health > 0 then
        SetVehicleHandlingFloat(veh, "CHandlingData", "fClutchChangeRateScaleDownShift",
            currentVehicleHandling["fClutchChangeRateScaleDownShift"] + shiftingtime)
        SetVehicleHandlingFloat(veh, "CHandlingData", "fClutchChangeRateScaleUpShift",
...
function setTires(veh)
    ...
    SetVehicleHandlingFloat(veh, "CHandlingData", "fLowSpeedTractionLossMult",
        currentVehicleHandling["fLowSpeedTractionLossMult"] + lowspeedtraction)
    SetVehicleHandlingFloat(veh, "CHandlingData", "fTractionCurveMin",
...
function setSuspension(veh)
    ...
    if currentVehicleParts["suspension"].health > 0 then
        SetVehicleHandlingFloat(veh, "CHandlingData", "fSuspensionRaise",
            currentVehicleHandling["fSuspensionRaise"] + height)
        SetVehicleHandlingFloat(veh, "CHandlingData", "fTractionCurveMax",
...
AddEventHandler("core_vehicle:sync", function(playerServerId, boostEnabled, purgeEnabled, lastVehicle)
...
exports("getNitro", function()
    if currentVehicleParts['nitro'] then
        return currentVehicleParts['nitro'].health
    else
        return 0
    end
end)
