...
CreateThread(function()
    local models = {'hei_prop_hei_lflts_02'}
    ...
    exports['target']:AddTargetModel(models, { -- Isso define os modelos, pode ser uma string ou uma tabela
        ...
        { -- Esta é a primeira tabela com opções, você pode fazer quantas opções quiser dentro da tabela de opções
            ...
            event = "GENESEETarget:AuroraCL", -- Este é o evento que será acionado ao clicar, este pode ser um evento de cliente, evento de servidor ou comando
            icon = 'fa-solid fa-scroll', -- Este é o ícone que será exibido ao lado desta opção de gatilho
            label = 'Ver tabela de preços' -- Este é o rótulo desta opção que você poderá clicar para acionar tudo, isso tem que ser uma string
...
RegisterNetEvent('GENESEETarget:AuroraCL')
AddEventHandler('GENESEETarget:AuroraCL', function()
  TriggerEvent('pz_aurora:openPriceMenu')
end)
